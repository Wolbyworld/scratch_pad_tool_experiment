#!/usr/bin/env python3
"""
Unit tests for mathematical functions in ScratchPadTools.
"""
...
class TestMathFunctions:
    ...
    @pytest.mark.unit
    def test_parse_expression_safely_basic(self, scratch_pad_tools):
        """Test basic expression parsing."""
        # Basic expressions
        result = scratch_pad_tools._parse_expression_safely("x + 2")
        assert str(result) == "x + 2"
        
        result = scratch_pad_tools._parse_expression_safely("2*x + 3")
        assert str(result) == "2*x + 3"
    
    @pytest.mark.unit
    def test_parse_expression_safely_caret_to_power(self, scratch_pad_tools):
        """Test conversion of ^ to ** for exponentiation."""
        result = scratch_pad_tools._parse_expression_safely("x^2")
        assert str(result) == "x**2"
        
        result = scratch_pad_tools._parse_expression_safely("2*x^3 + x^2")
        assert str(result) == "2*x**3 + x**2"
    
    @pytest.mark.unit
    def test_parse_expression_safely_implicit_multiplication(self, scratch_pad_tools):
        """Test implicit multiplication handling."""
        # 3x should become 3*x
        result = scratch_pad_tools._parse_expression_safely("3x")
        assert str(result) == "3*x"
        
        # Multiple variables
        result = scratch_pad_tools._parse_expression_safely("2x + 3y")
        assert str(result) == "2*x + 3*y"
        
        # With parentheses: )x should become )*x
        result = scratch_pad_tools._parse_expression_safely("(x+1)y")
        assert str(result) == "(x + 1)*y"
    
    @pytest.mark.unit
    def test_parse_expression_safely_preserve_functions(self, scratch_pad_tools):
        """Test that function names are preserved (sin, cos, etc.)."""
        result = scratch_pad_tools._parse_expression_safely("sin(x)")
        assert str(result) == "sin(x)"
        
        result = scratch_pad_tools._parse_expression_safely("cos(x) + sin(x)")
        assert str(result) == "sin(x) + cos(x)"
        
        result = scratch_pad_tools._parse_expression_safely("log(x) + exp(x)")
        assert str(result) == "exp(x) + log(x)"
    
    @pytest.mark.unit
    def test_parse_expression_safely_invalid_expression(self, scratch_pad_tools):
        """Test handling of invalid expressions."""
        with pytest.raises(ValueError, match="Invalid mathematical expression"):
            scratch_pad_tools._parse_expression_safely("invalid_expression_with_$%@")
    
    @pytest.mark.unit
    def test_solve_equation_basic(self, scratch_pad_tools, sample_test_cases):
        """Test basic equation solving."""
        for equation, variable, expected_solutions in sample_test_cases["equations"]:
            result = scratch_pad_tools.solve_equation(equation, variable)
    ...
    @pytest.mark.unit
    def test_solve_equation_without_equals(self, scratch_pad_tools):
        """Test solving equations without explicit = sign (assumes = 0)."""
        result = scratch_pad_tools.solve_equation("x**2 - 4", "x")
        
        assert result["status"] == "success"
        assert sorted(result["solutions"]) == ["-2", "2"]
    
    @pytest.mark.unit
    def test_solve_equation_no_solutions(self, scratch_pad_tools):
        """Test equation with no real solutions."""
        result = scratch_pad_tools.solve_equation("x**2 + 1 = 0", "x")
    ...
    @pytest.mark.unit
    def test_solve_equation_invalid(self, scratch_pad_tools):
        """Test handling of invalid equations."""
        result = scratch_pad_tools.solve_equation("invalid$equation", "x")
        
        assert result["status"] == "error"
        assert "Invalid mathematical expression" in result["message"]
    
    @pytest.mark.unit
    def test_simplify_expression_basic(self, scratch_pad_tools, sample_test_cases):
        """Test basic expression simplification."""
        for expression, expected in sample_test_cases["expressions"]:
            result = scratch_pad_tools.simplify_expression(expression)
    ...
    @pytest.mark.unit
    def test_simplify_expression_already_simplified(self, scratch_pad_tools):
        """Test simplifying an already simplified expression."""
        result = scratch_pad_tools.simplify_expression("x + 1")
    ...
    @pytest.mark.unit
    def test_simplify_expression_invalid(self, scratch_pad_tools):
        """Test handling of invalid expressions for simplification."""
        result = scratch_pad_tools.simplify_expression("invalid$expression")
        
        assert result["status"] == "error"
        assert "Invalid mathematical expression" in result["message"]
    
    @pytest.mark.unit
    def test_calculate_derivative_basic(self, scratch_pad_tools, sample_test_cases):
        """Test basic derivative calculations."""
        for expression, variable, order, expected in sample_test_cases["derivatives"]:
            result = scratch_pad_tools.calculate_derivative(expression, variable, order)
    ...
    @pytest.mark.unit
    def test_calculate_derivative_first_order_default(self, scratch_pad_tools):
        """Test that first order is default for derivatives."""
        result = scratch_pad_tools.calculate_derivative("x**3")
    ...
    @pytest.mark.unit
    def test_calculate_derivative_higher_order(self, scratch_pad_tools):
        """Test higher order derivatives."""
        result = scratch_pad_tools.calculate_derivative("x**4", "x", 2)
    ...
    @pytest.mark.unit
    def test_calculate_derivative_invalid(self, scratch_pad_tools):
        """Test handling of invalid expressions for derivatives."""
        result = scratch_pad_tools.calculate_derivative("invalid$expression")
        
        assert result["status"] == "error"
        assert "Invalid mathematical expression" in result["message"]
    
    @pytest.mark.unit
    def test_calculate_integral_indefinite(self, scratch_pad_tools):
        """Test indefinite integral calculations."""
        result = scratch_pad_tools.calculate_integral("x**2", "x")
    ...
    @pytest.mark.unit
    def test_calculate_integral_definite(self, scratch_pad_tools):
        """Test definite integral calculations."""
        result = scratch_pad_tools.calculate_integral("x**2", "x", ["0", "1"])
    ...
    @pytest.mark.unit
    def test_calculate_integral_invalid_limits(self, scratch_pad_tools):
        """Test handling of invalid limits for definite integrals."""
        result = scratch_pad_tools.calculate_integral("x**2", "x", ["0"])  # Only one limit
        
        assert result["status"] == "error"
        assert "Limits must be a list of exactly 2 values" in result["message"]
    
    @pytest.mark.unit
    def test_calculate_integral_invalid_expression(self, scratch_pad_tools):
        """Test handling of invalid expressions for integration."""
        result = scratch_pad_tools.calculate_integral("invalid$expression")
        
        assert result["status"] == "error"
        assert "Invalid mathematical expression" in result["message"]
    
    @pytest.mark.unit
    def test_factor_expression_basic(self, scratch_pad_tools, sample_test_cases):
        """Test basic polynomial factoring."""
        for expression, expected in sample_test_cases["factors"]:
            result = scratch_pad_tools.factor_expression(expression)
    ...
    @pytest.mark.unit
    def test_factor_expression_already_factored(self, scratch_pad_tools):
        """Test factoring an already factored expression."""
        result = scratch_pad_tools.factor_expression("(x + 1)*(x + 2)")
    ...
    @pytest.mark.unit
    def test_factor_expression_irreducible(self, scratch_pad_tools):
        """Test factoring an irreducible polynomial."""
        result = scratch_pad_tools.factor_expression("x**2 + 1")
    ...
    @pytest.mark.unit
    def test_factor_expression_invalid(self, scratch_pad_tools):
        """Test handling of invalid expressions for factoring."""
        result = scratch_pad_tools.factor_expression("invalid$expression")
        
        assert result["status"] == "error"
        assert "Invalid mathematical expression" in result["message"]
    
    @pytest.mark.unit
    def test_calculate_complex_arithmetic_basic(self, scratch_pad_tools, sample_test_cases):
        """Test complex arithmetic calculations."""
        for expression, expected in sample_test_cases["arithmetic"]:
            result = scratch_pad_tools.calculate_complex_arithmetic(expression)
    ...
    @pytest.mark.unit
    def test_calculate_complex_arithmetic_large_numbers(self, scratch_pad_tools):
        """Test arithmetic with very large numbers."""
        result = scratch_pad_tools.calculate_complex_arithmetic("999999999*888888888")
    ...
    @pytest.mark.unit
    def test_calculate_complex_arithmetic_decimal_precision(self, scratch_pad_tools):
        """Test arithmetic with high decimal precision."""
        result = scratch_pad_tools.calculate_complex_arithmetic("1.123456789*2.987654321")
    ...
    @pytest.mark.unit
    def test_calculate_complex_arithmetic_invalid_expression(self, scratch_pad_tools):
        """Test handling of invalid arithmetic expressions."""
        result = scratch_pad_tools.calculate_complex_arithmetic("invalid_expression")
...
class TestExpressionParsingSecurity:
    ...
    @pytest.mark.unit
    def test_dangerous_input_filtered(self, scratch_pad_tools):
        ...
        for dangerous_input in dangerous_inputs:
            ...
            try:
                result = scratch_pad_tools._parse_expression_safely(dangerous_input)
    ...
    @pytest.mark.unit
    def test_only_mathematical_characters_allowed(self, scratch_pad_tools):
        ...
        try:
            result = scratch_pad_tools._parse_expression_safely(input_expr)
